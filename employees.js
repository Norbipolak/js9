...
let lastName = "";
let salaryFrom = 0;
let salaryTo = Number.MAX_SAFE_INTEGER;

firstNameInput.addEventListener("input", function () {
    firstName = this.value;
    search(firstName, lastName, department, salaryFrom, salaryTo);//hozzágyuk ide a department
});
...
salaryFrom.addEventListener("input", function () {
    salaryFrom = parseInt(this.value);
    if (isNaN(salaryFrom))
        salaryFrom = 0;
    search(firstName, lastName, department, salaryFrom, salaryTo)
});
salaryTo.addEventListener("input", function () {
    salaryTo = parseInt(this.value);
    if (isNaN(salaryTo))
        salaryFrom = Number.MAX_SAFE_INTEGER;
    search(firstName, lastName, department, salaryFrom, salaryTo)
});
/*
...
/*
search függvény ugyanugy kap megkapja a salaryFrom-ot értékként
*/
function search(firstName, lastName, department, salaryFrom, salaryTo) {
    const filteredEmployees =
        employees.filter((employee) =>
            employee.firstName.toLoweCase().includes(firstName)
            && employee.lastName.toLoweCase().includes(lastName)
            && employee.department.includes(department)
            && employee.salary >= salaryFrom /*X*/
            && employee.salary <= salaryTo) /*X*/
}
/*
midegyik, eventListener-nél, amit csináltunk, kiegészítjük még a meghívását a függvénynek a salaryFrom-val 
search(firstName, lastName, department, salaryFrom)

és most rá tudunk keresni, ha a salary (-tól) mezőbe pl. beírjuk, hogy 1000000, akkor azokat adja ki, akinek a fizetése e felett van.
amiatt, amit meghatárotunk a a search függvény filteredEmployees-ében -> && employee.salary >= salaryFrom
*/
/*
A salaryTo majdnem teljesen ugyanazm mint a salaryFrom, annyi különbséggel, hogy a filteredEmployeesban -> && employee.salary <= salaryTo
ennek is csináltunk egy eventListenert és kiegészítettük a search függvényt -> search(firstName, lastName, department, salaryFrom)
amit mindegyik eventListenernél meghívtunk.
Ha viszont azt mondjuk a salaryTo eventListenerében, hogy salaryTo = 0; mint a salaryFrom-nál, mert ha üresen haggyuk, (ha egyszer beírtam 
de utána kitörlöm) akkor 0 és semmit nem fog megjeleniteni, mert senkinek a fizetése nem kisebb vagy egyenlő, mint 0 ->
itt vagy megnézzük, hogy kinek a fizetése a legnagyobb és megaggyuk azt vagy egy olyan magas értéket adunk meg neki ami biztos már nem lehet
-> 
erre jó a Number_Safe_Integer, ami az értéke -> 90071999254740992 és beállítjuk a salaryTo ezt a kezdőértéket 
*/

let salaryTo = Number.MAX_SAFE_INTEGER; // elötte fontos, hogy a let-nek is megadjuk ezt a Number.MAX_SAFE_INTEGER értéket

salaryTo.addEventListener("input", function () {
    salaryTo = parseInt(this.value);
    if (isNaN(salaryTo))
        salaryFrom = Number.MAX_SAFE_INTEGER; /*X*/
    search(firstName, lastName, department, salaryFrom, salaryTo)
});
/*
és ezzel meg van a client side search 
még annyi, hogy lehetne szebb is a felépítése (ami az elején van) és ezért csinálunk majd egy objektumba ->
amihez készítünk egy ujabb random-employees.js-et
*/
/*****************************************************************************************/
